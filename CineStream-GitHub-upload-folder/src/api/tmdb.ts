import type { Credit, FallbackDetails, Keyword, Movie, MovieBundle, MovieDetails, MovieImage, Review, Video, WatchProvider, WatchProviderGroups } from "../types";
...
type WatchProvidersResponse = {
  results?: Record<string, WatchProviderCountryResult>;
};

type WatchProviderCountryResult = {
...
export async function getMovieBundle(movieId: number): Promise<MovieBundle> {
  ...
  const watchProviders = isUpcomingRelease(details) ? emptyWatchProviders() : await getMovieWatchProviders(movieId);
...
export async function getMovieWatchProviders(movieId: number): Promise<WatchProviderGroups> {
  try {
    const data = await tmdbFetch<WatchProvidersResponse>(`/movie/${movieId}/watch/providers`, { language: "" }, DETAILS_CACHE_MS);
    const indiaProviders = data.results?.IN;

    return {
      stream: sortProviders(indiaProviders?.flatrate ?? []),
      rent: sortProviders(indiaProviders?.rent ?? []),
      buy: sortProviders(indiaProviders?.buy ?? []),
      link: indiaProviders?.link
    };
  } catch {
    return emptyWatchProviders();
  }
}

function emptyWatchProviders(): WatchProviderGroups {
