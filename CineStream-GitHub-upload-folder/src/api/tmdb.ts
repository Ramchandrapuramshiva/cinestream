...
export async function getUpcomingMovies() {
  const data = await tmdbFetch<MovieResponse>("/movie/upcoming", { page: "1" });
  return rankUpcomingMovies(data.results.filter(isUpcomingRelease)).filter(hasArtwork);
}
...
function rankUpcomingMovies(movies: Movie[]) {
  return [...movies].sort((a, b) => {
    ...
    return dateA - dateB || b.vote_average - a.vote_average;
  });
}
